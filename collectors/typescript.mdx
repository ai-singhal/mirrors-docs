---
title: "TypeScript SDK for AI agent tracing"
sidebarTitle: "TypeScript"
description: "Wrap your AI SDK, Anthropic, OpenAI or LangChain.js client with mirrorkit and your Node agent traces stream to Mirrors, non-blocking and never throwing."
icon: "node-js"
---
...
Run `npm install mirrorkit`. It requires **Node ≥ 18** (it uses the built-in
`fetch`) and has zero runtime dependencies of its own.
...
Call `init()` with your workspace key and project name, then wrap each SDK client
once with `instrument()`. That is the whole integration: two lines plus one wrap
per client, and no change to how you call the model.
